...
public class Path extends ArrayList<PathNode> { 
	 ...
	 * @param pman pacman
	 */
	public Path(ArrayList<Fruit> fList,Player pman) { // constructor
		...
		for(int i=0; i<sortedFL.size();i++) {
			if (i==0) { // if i is zero runTime is between pacman and the first fruit
				runTime=(coords.distance3d(pman.getPoint(), sortedFL.get(i).getPoint())/20); //"distance"/"pacman speed".
				this.add(new PathNode(sortedFL.get(i),sortedFL.get(i).getID(),runTime)); //add new path node to the path 
			}
			else {// if i is bigger then zero runTime is between the previous fruit and the current fruit.
				runTime+=(coords.distance3d(sortedFL.get(i-1).getPoint(), sortedFL.get(i).getPoint())/20);
				this.add(new PathNode(sortedFL.get(i),sortedFL.get(i).getID(),runTime));
			}
		}
	}
	 ...
	 */
	private ArrayList<Fruit> sortByTime(ArrayList<Fruit>fList,Player pman) {
